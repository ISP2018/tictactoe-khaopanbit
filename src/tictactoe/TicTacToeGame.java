...
/** The model for control moves of tictactoe game **/
...
public class TicTacToeGame {
	...
	public TicTacToeGame(int size) {
		...
		pieces = new Piece[boardsize][boardsize]; // stores info about pieces on
													// board
	...
	public void startNewGame() {
		...
		// Remove Pieces from the board (view), but not the squares themselves.
		// Use a Predicate to test for Piece.
	...
	/**
	 * Place a piece at a given (row,col) on the game board. It is up to the
	 * caller to make sure that the cell can be occupied before calling moveTo.
	 * 
	 * @param piece
	 *            the piece to place
	 * @param row
	...
	public void moveTo(Piece piece, int col, int row) {
		...
		board.add(piece, col, row); // GridPane.add has column param before row
									// param
		...
		if (winner() != Player.NONE || boardIsFull())
			gameOver.set(true);
	...
	/**
	 * Get the GameOver property. This enables adding observers to the gameOver
	 * property.
	 * 
